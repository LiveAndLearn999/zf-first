        ...
        <el-dialog  
            ...
            <el-form :model="DetailFormData" label-width="100px" label-position="right">
                <el-form-item label="货物名称:">{{DetailFormData.name || '---'}}</el-form-item>
                 <el-form-item label="货物类型:">{{DetailFormData.type_name  || '---'}}</el-form-item>
                  <el-form-item label="货物包装类型:">{{DetailFormData.pack_type_name || '---'}}</el-form-item>
...
<script>
    ...
    export default {
        ...
        mounted() {
         lime.req('EbPackTypeList',{login_token : lime.cookie_get('login_token')}).then(res => {
             this.type_value = res.data.rows[0].uuid
            this.typeAry = res.data.rows.map(v => {
                return {value: v.uuid, label: v.type_name, ...v}
            });
        })
         lime.req('EbGoodsTypeList',{login_token : lime.cookie_get('login_token')}).then(res => {
             this.good_value = res.data.rows[0].uuid
        ...
        methods: {
            ...
            onAddSubmit() {
                let pam = {
                    ...
                    pack_type_uuid: this.type_value,
                    type_uuid: this.good_value
                }
                EbGoodAdd(pam, res => {
                    this.init();
                    this.add_show = false;
            ...
            handleEdit() {
                ...
                this.edit_good_value = this.curr_row.type_uuid
                this.edit_type_value = this.curr_row.pack_type_uuid
                this.edit_show = true
            },
            onEditSubmit() {
                let pam = {
